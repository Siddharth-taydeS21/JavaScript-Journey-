// console.log("hey today i am learning Js Functions")

function myfunction(a, b) {
    return a * b;
}
let result = myfunction(3, 4);
// console.log(result)

let result2 = myfunction(5, 2)
// console.log(result2)

let result3 = myfunction(25, 4)
// console.log(result3)


//example 1 :
function Celsiusto(franahite) {
    return (5 / 9) * (franahite - 32);
}
let todaysTempreture = Celsiusto(105)
// console.log(todaysTempreture)

//example 2:
function addition(a, b) {
    return a + b;
}
let value = addition(5, 7)
// console.log(value)

//example 3:
function isEven(num) {
    if (num % 2 === 0) {
        return "Even";
    }
    else {
        return "Odd";
    }
}
let fraction = isEven(40)
// console.log(fraction)

//example 4 :
function square(num) {
    return num * num;
}
let findsquare = square(50)
// // console.log(findsquare)

//example 5 :
function max(a, b) {
    if (a > b) {
        return a;
    }
    else {
        return b;
    }
}
let findmax = max(44, 77)
// console.log(findmax)

//example 6 :
function welcome(str) {
    return "Hellow " + str + "!";
}
//let guestname = welcome("Mr. Sowftware Engineer")
//console.log(guestname)
//alternative or short version :
// console.log(welcome("Mr. Sowftware Engineer"))

// Arrow Functions :
// Arrow functions are basically the shorter version of writing a function. with help of arrow function you can store a function in a variable

//Arrow Functions Examples :
// example 1 :
let toCelsius = (number) => { return (5 / 8) * (number - 32) };
let Tempreture = toCelsius(68);
// console.log(Tempreture)

//example 2 :
let maximum = (a, b) => {
    if (a > b){
        return a;
    }
    else{
        return b;
    }
}
// console.log(maximum(50, 230))

//advance example with implicit return:
//1.
let minimum = (a, b) => a > b ? `${a} is greater that ${b}` : `${b} is greater that ${a}`;
// console.log(minimum(56, 120))

//2.
const welcomMassage  = (str) => `hello ${str}!`
// console.log(welcomMassage("Sid"));
 

//3. 
const gradeCheck = (num) =>
  num >=  90 ? ("A+")
: num >=  80 ? ("A")
: num >=  70 ? ("B")
: num >=  60 ? ("C")
: "D";

console.log(gradeCheck(70));


//Immediately Invoked Function Expressions -
//basic syntax -  
/* 
normal functions-

functionName = {
//defination code here..
};
functionNmae(); -//function call

Immediately Invoked Function Expression-

(functionName = {
//defination code here..
})(); -//function call (Immediately Invoked)


TIP - DON'T FORGET TO PUT A SEMICOLLON ";" AFTER THE IIFE FUNCTION. IT CAN CAUSE ERRORS...

*How to pass parameters and arguments in an IIFE Function ? -
it's same as the normals -

(functionNmae(parameters){
//defination code here..
})(arguments) 

*/